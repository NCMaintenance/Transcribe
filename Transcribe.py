...
# PDF function create_pdf_from_structured_summary removed
...
# PDF function create_pdf_from_narrative_summary removed
...
if st.session_state.structured_summary and not st.session_state.is_loading_structured:
    ...
    # Download button for Structured Summary (DOCX only)
    st.markdown("<h5>Download Structured Summary:</h5>", unsafe_allow_html=True)
    col1, col_spacer = st.columns([1,3]) # Only one download button now
    with col1:
        try:
    ...
    # Generate Doctor's Narrative Summary Button 
...
if st.session_state.doctors_narrative_summary and not st.session_state.is_loading_narrative:
    ...
    # Download button for Narrative Summary (DOCX only)
    st.markdown("<h5>Download Narrative Summary:</h5>", unsafe_allow_html=True)
    col_narr_1, col_narr_spacer = st.columns([1,3]) # Only one download button now
    with col_narr_1:
        try:
